...
export function createPreviewAgentSettingsPanel(options: {
  ...
  const joystick = mkCheck(
    "Enable joystick",
    "joystickEnabled",
    "Requires debug mode. Drag the on-canvas stick to move the first agent in world space."
  );

  const syncJoystickDisabled = (): void => {
    joystick.input.disabled = !getPreviewViewSettings().debugMode;
  };

  syncJoystickDisabled();
  debug.input.addEventListener("change", () => {
    syncJoystickDisabled();
  });
