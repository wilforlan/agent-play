...
import { WORLD_JOURNEY_EVENT } from "./play-transport.js";
...
describe("PlayWorld snapshot and session", () => {
  ...
  it("getSnapshotJson includes players and serializes dates in lastUpdate", async () => {
    ...
    const p = await world.addPlayer({
      name: "a1",
      type: "langchain",
      agent: { type: "langchain", toolNames: ["t1"] },
    });
  ...
  it("last journey wins for same player", async () => {
    ...
    const p = await world.addPlayer({
      name: "p",
      type: "langchain",
      agent: { type: "langchain", toolNames: [] },
    });
  ...
  it("emits WORLD_JOURNEY_EVENT with payloads", async () => {
    const world = new PlayWorld({});
    await world.start();
    const p = await world.addPlayer({
      name: "p",
      type: "langchain",
      agent: { type: "langchain", toolNames: ["x"] },
    });
    let count = 0;
    world.on(WORLD_JOURNEY_EVENT, () => {
      count += 1;
    });
    const s = new Date();
    world.recordJourney(p.id, {
