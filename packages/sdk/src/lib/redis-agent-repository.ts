...
function agentsIndexKey(hostId: string): string {
  return `agent-play:${hostId}:agents`;
}
...
export class RedisAgentRepository implements AgentRepository {
  ...
  async createAgent(
    ...
    pipe.sadd(agentsIndexKey(this.hostId), agentId);
    await pipe.exec();
  ...
  async listAgents(): Promise<StoredAgentRecord[]> {
    const ids = await this.redis.smembers(agentsIndexKey(this.hostId));
  ...
  async deleteAgent(agentId: string): Promise<boolean> {
    ...
    pipe.srem(agentsIndexKey(this.hostId), agentId);
