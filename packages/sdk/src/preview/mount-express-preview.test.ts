...
describe("mountExpressPreview", () => {
  ...
  beforeAll(() => {
    if (!existsSync(join(assetsDir, "index.html"))) {
      throw new Error(
        "play-ui not built: run `npm run build:play-ui` from the repository root"
      );
    }
  });
  ...
  it("returns snapshot JSON for valid sid", async () => {
    ...
    await world.addPlayer({
      name: "p1",
      type: "langchain",
      agent: { type: "langchain", toolNames: ["chat_tool", "t"] },
    });
    mountExpressPreview(app, world, { basePath: "/agent-play", assetsDir });
    const res = await request(app)
      .get("/agent-play/snapshot.json")
  ...
  it("records proximity action for valid players", async () => {
    const app = express();
    const world = new PlayWorld({});
    await world.start();
    const sid = world.getSessionId();
    const a = await world.addPlayer({
      name: "a",
      type: "langchain",
      agent: { type: "langchain", toolNames: ["chat_tool", "t"] },
    });
    const b = await world.addPlayer({
      name: "b",
      type: "langchain",
      agent: { type: "langchain", toolNames: ["chat_tool", "t2"] },
    });
    mountExpressPreview(app, world, { basePath: "/agent-play", assetsDir });
    const res = await request(app)
      .post("/agent-play/proximity-action")
      .query({ sid })
      .send({
        fromPlayerId: a.id,
        toPlayerId: b.id,
    ...
    const pa = snap.players.find((p) => p.playerId === a.id);
    const pb = snap.players.find((p) => p.playerId === b.id);
    expect(pa?.recentInteractions?.some((i) => i.text.includes("Assist"))).toBe(
      true
    );
    expect(pb?.recentInteractions?.some((i) => i.text.includes("Assist"))).toBe(
      true
    );
  });
});
