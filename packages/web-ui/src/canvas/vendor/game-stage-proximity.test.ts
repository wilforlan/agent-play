...
describe("game-stage-proximity", () => {
  ...
  it("hidden gems exposes the next chest and exit door as proximity targets", () => {
    ...
    expect(targets.some((target) => target.label === "Chest")).toBe(true);
    expect(
  ...
  it("delivery dash exposes the courier as a proximity target", () => {
    const stage = buildGameDeliveryDashStage({ cellScale: 48 });
    const targets = stage.listProximityTargets?.() ?? [];
    expect(targets.some((target) => target.id === "courier")).toBe(true);
    expect(targets.some((target) => target.label === "Courier")).toBe(true);
    expect(targets.some((target) => target.verb === "Move")).toBe(true);
  });
});
